...
asset_root = "../../assets"
shadow_hand_asset_file = "mjcf/open_ai_assets/hand/shadow_hand1.xml"
object_asset_file = "dataset_apien/3614/mobility.urdf"
...
total_data = np.load('one_hand_data.npy', allow_pickle=True).item()
...
shadow_hand_actor = gym.create_actor(env, shadow_hand_asset, shadow_another_hand_start_pose, 'shadow_hand', 0, 1)
...
for index in range(right_hand_rot.shape[0]):
    ...
    root_state_tensor_torch[shadow_hand_actor_idx, :3] = right_hand_pos
    ...
    object_pos = torch.tensor([object_trans[index][0]+ 14, object_trans[index][1], object_trans[index][2] + 2], dtype=torch.float32)
